...
public:
    ...
    ThreadPool(ConcurrencyModel concurrency_model_t, ConnectionPool *sql_conn_pool_t, int threadpool_max_t = 8, int requests_list_max_t = 10000);
    ~ThreadPool();
...
private:
    ...
    ConnectionPool *sql_conn_pool_;      //数据库连接池
    ConcurrencyModel concurrency_model_; //模型切换
};

template <typename T>
ThreadPool<T>::ThreadPool(ConcurrencyModel concurrency_model_t, ConnectionPool *sql_conn_pool_t, int threadpool_max_t, int requests_list_max_t)
    ...
    while (true)
    {
        ...
        if (REACTOR_MODEL == concurrency_model_)
        {
            ...
            if (0 == request->http_state_)
            {
                ...
                {
                    request->completed_flag_ = 1; //告诉主线程已经读取完
                    request->close_conn_flag_ = 1;  //因为失败了，所以标记超时，清除该连接
                }
            }
            //写状态
            else
